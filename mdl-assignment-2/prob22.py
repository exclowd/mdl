...
if __name__ == '__main__':
    initStates()
    Actions = []
    chosen_actions = ["" for x in range(len(States))]
        ...
        for i in range(len(States)):
            ...
            for action in Actions[i]:
                ...
                if sm > maxUtility:
                    maxUtility = sm
                    chosen = action
            chosen_actions[i] = chosen
            print(
                f"({States[i][1]},{States[i][2]},{States[i][3]},{States[i][4]},{States[i][5]}):{chosen}=[{temp[i]:.3f}]")
    ...
    # for i in range(len(States)):
    #     print(States[i][1:], chosen_actions[i])
    # for i in range(len(States)):
    #     for act in Actions[i].keys():
    #         for next_state in Actions[i][act]:
    #             print(States[i][1:] , '-> ', States[next_state[0]][1:], ': ', act, ': ', next_state[1])
    #         print()

    cur_state = getid(('C', 1,1, 'D', 100))
    cost = 0
    while True:
        cur_act = chosen_actions[cur_state]
        if cur_act == 'NONE':
            break
        probs = np.array([x[1] for x in Actions[cur_state][cur_act]])
        next_states = np.array([x for x in range(len(Actions[cur_state][cur_act]))])
        choice = np.random.choice(next_states, p=probs)
        next_state = Actions[cur_state][cur_act][choice][0]
        cost += Actions[cur_state][cur_act][choice][2]
        print(States[cur_state][1:], ' -> ',  States[next_state][1:], ': ', cur_act, '= ', cost)
